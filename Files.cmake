...
function(s_should_add_file OUTPUT PATH)
    ...
    s_hoist_variable(${OUTPUT})
endfunction()

function(s_add_file_force PATH)
    s_clean_file_path(PATH ${PATH})
    list(APPEND S_CURRENT_PROJECT_SOURCES ${PATH})
    s_hoist_variable(S_CURRENT_PROJECT_SOURCES)
endfunction()

function(s_add_file PATH)
    s_clean_file_path(PATH ${PATH})
    s_should_add_file(SHOULD_BE_ADDED ${PATH})
    if (SHOULD_BE_ADDED)
        list(APPEND S_CURRENT_PROJECT_SOURCES ${PATH})
        s_hoist_variable(S_CURRENT_PROJECT_SOURCES)
    endif()
...
function(s_del_file PATH)
    s_clean_file_path(PATH ${PATH})
    list(REMOVE_ITEM S_CURRENT_PROJECT_SOURCES ${PATH})
    s_hoist_variable(S_CURRENT_PROJECT_SOURCES)
endfunction()

function(s_add_dir_non_recursive PATH)
    aux_source_directory(${PATH} TARGET_DIR_SOURCES)
    foreach(TARGET_FILE ${TARGET_DIR_SOURCES})
        s_add_file(${TARGET_FILE})
    endforeach()
    s_hoist_variable(S_CURRENT_PROJECT_SOURCES)
endfunction()

function(s_add_dir_recursive PATH)
    ...
    foreach(DIR_PATH ${DIR_LIST})
        s_add_dir_non_recursive(${DIR_PATH})
    endforeach()
    s_hoist_variable(S_CURRENT_PROJECT_SOURCES)
endfunction()

function(s_add_dir PATH)
    cmake_parse_arguments(PARSE_ARGV 1 DIR "RECURSIVE" "" "")
    if (${DIR_RECURSIVE})
        s_add_dir_recursive(${PATH})
    else()
        s_add_dir_non_recursive(${PATH})
    endif()
    s_hoist_variable(S_CURRENT_PROJECT_SOURCES)
endfunction()
